...
def getConturs(img):
    ...
    for cnt in contours:
        ...
        if area > 500:
            ...
            print(len(approx))  # shows the number of corners
            objCor = len(approx)
            ...
            if objCor == 3:
                ...
                aspRatio = w/float(h)
                if aspRatio >0.95 and aspRatio <1.05: # 5% margin
                    objectType = "Square"
                else:
            ...
            cv2.rectangle(imgContour,(x,y),(x+w,y+h),(0,255,0),3) # draw the box
            cv2.putText(imgContour,objectType,
                        (x+(w//2)-30,y+(h//2)-10), cv2.FONT_HERSHEY_COMPLEX,1,
                        (0,0,0),2)  # specify the shape
